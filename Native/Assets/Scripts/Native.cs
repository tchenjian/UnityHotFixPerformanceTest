...
public static class Native
{
    ...
    private static MethodInfo checkMethod;
    ...
    private static bool isNative = true;

    ///<summary>
    /// 第一次刷新迷雾(全量)
    ///</summary>
    public static void TestFunc1 (int[] fogInfo, int unlockCount,bool _isNative)
    {
        isNative = _isNative;
        Debug.LogError (isNative ? "Native":"WooLoong");
        // SetFogOfWarAlpha(_fogVolume, new Vector3(6144,60.0f,6144), 12288, 1, 0);
        ...
        for (int i = 0; i < _serverFogInfo.Length; i++)
        {
            for (int j = 0; j < _bitCount; j++)
            {
                ...
                if (bUnlock)
                {
                    ...
                    //这个点有迷雾解锁
                    // var cellPos = new Vector2 ((col + 0.5f) * _mapFogSize * 100, (row + 0.5f) * _mapFogSize * 100);
                    // var worldPos = MapCoordUtil.CellPosToWorldPos (cellPos);
                    // SetFogOfWarAlpha (_fogVolume, worldPos, _mapFogSize, 0, 0);
                }
            }
        }
